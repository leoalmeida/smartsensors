...
let Messenger = mongoose.model('Messenger');

const ctrl = {};

ctrl.startMotion = (equipment, socket) => {
    ...
    object.on("change", function (data) {
        ...
        publishReadings(lastUpdate, object);
...
ctrl.startSensor = (equipment, socket) => {
...
ctrl.startFlow = (equipment, socket) => {
...
ctrl.startHygrometer = (equipment, socket) => {
...
ctrl.startThermometer = (equipment, socket) => {
...
ctrl.startLight = (equipment, socket) => {
    ...
    object.on("change", function() {
      ...
      this.lastReading = this.level;

      this.percentage = Math.round(100 - (this.level * 100));
...
ctrl.startRelay = (equipment, socket) => {
...
ctrl.startLed = (equipment, socket) => {
...
//------ helper function
function publishReadings(reading,object){
  if (!reading) console.log({ data: reading, code: 422, messageKeys: ['not-found'] });
  let newVal = new Messenger({ root: object.id, data: reading });
  //console.log(newVal);
