 ...
 */
var Sensor = function (objectName, methods) {
 ...
 */
Sensor.ensureImplements = function (object) {
    // Check that the right amount of arguments are provided
    if (arguments.length < 2) {
        throw new Error ("Sensor.ensureImplements was called with " + arguments.length + "arguments, but expected at least 2.");
    }
    ...
    for (var i = 1, len = arguments.length; i < len; i++) {
        // Check the object provided as an argument is an instance of the 'Sensor' class
        var interface = arguments[i];
        if (interface.constructor !== Sensor) {
            throw new Error ("Sensor.ensureImplements expects the second argument to be an instance of the 'Sensor' constructor.");
        }

        // Otherwise if the provided argument IS an instance of the Sensor class then
...
// We pass into the Sensor the name of the current Sensor instance,
// followed by an Array of the methods we are expecting to find
var test = new Sensor('test', ['details', 'age']);
...
function Person(config) {
    ...
    // followed by the name of the Sensor instance that we're checking against
    Sensor.ensureImplements(config.actions, test);
